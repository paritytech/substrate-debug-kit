...
pub async fn get_identity(who: &AccountId, client: &Client, at: Hash) -> String {
	...
	let maybe_subidentity = storage::read::<(AccountId, Data)>(
		storage::map_key::<Blake2_128Concat>(b"Identity", b"SuperOf", who.as_ref()),
		client,
		at,
	).await;

	let maybe_identity = storage::read::<Registration<Balance>>(
		storage::map_key::<Twox64Concat>(b"Identity", b"IdentityOf", maybe_subidentity.as_ref().map_or(who.as_ref(), |x| x.0.as_ref())),
		client,
		at,
	).await;
	...
	if let Some(identity) = maybe_identity {
		...
		let result = match display {
			Data::Raw(bytes) => format!("{}", Colour::Yellow.bold().paint(String::from_utf8(bytes).expect("Identity not utf-8"))),
			_ => format!("{}", Colour::Red.bold().paint("???")),
		};
		if let Some(sub_identity) = maybe_subidentity {
			match sub_identity.1 {
				Data::Raw(bytes) => format!("{} ({})", result, Colour::Yellow.paint(String::from_utf8(bytes).expect("Identity not utf-8"))),
